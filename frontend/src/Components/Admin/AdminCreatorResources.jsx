...
const AdminCreatorResources = () => {
  ...
  // API functions
    ...
    const handleSubmit = async (e) => {
      ...
      if (pdfFile && formData.type === 'pdf') {
        console.log('Adding PDF file to form data');
      ...
      try {
        let result;
        if (editingResource) {
          console.log('Updating existing resource:', editingResource._id);
          result = await apiRequest(`/api/creator-resources/${editingResource._id}`, {
            method: 'PUT',
            body: submitFormData
          });
        } else {
          console.log('Creating new resource');
          result = await apiRequest('/api/creator-resources', {
            method: 'POST',
            body: submitFormData
          });
        }
