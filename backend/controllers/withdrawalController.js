...
export const getPendingWithdrawals = async (req, res) => {
  try {
    ...
    // After fetching verification for each withdrawal, add this:
    const verification = await Verification.findOne({ user: withdrawal.user._id });
    console.log(`Verification for user ${withdrawal.user._id}:`, verification);
    console.log('Documents in verification:', verification?.documents);
    res.status(500).json({
      success: false,
